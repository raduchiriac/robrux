...
export function withApollo(PageComponent, { ssr = true } = {}) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    // https://github.com/zeit/next.js/pull/8556#discussion_r324106988
    if (displayName === 'App') {
      console.log('[🚨] This withApollo HOC only works with Page Components.');
    }
...
function createApolloClient(initialState = {}, { getToken }) {
  ...
  let uri = '';
  if (process.env.NODE_ENV !== 'production') {
    uri = `${process.env.HOSTNAME}:${process.env.PORT}${process.env.GRAPHQL_ROUTE}`;
  } else {
    uri = `${process.env.HOSTNAME}${process.env.GRAPHQL_ROUTE}`;
  }

  console.log('[🚨]PORT:', process.env.PORT, ',length', process.env.PORT.length);
  console.log('uri', uri);
